...
(function () {
    ...
    function enhance(contentTd, sidebarTd) {
        ...
        if (!isArticlesPage && !isIndexPage) {
            setupPrevNextNav(main).catch(() => { });
        }
    }

    // Fetches /ind.html once (session-cached), finds the current essay, appends
    // a prev/next footer nav. Ordering follows doc order on ind.html.
    async function setupPrevNextNav(main) {
        const CACHE_KEY = 'pg-reader-articles-list';
        const currentPath = location.pathname.replace(/^\//, '').toLowerCase();
        if (!currentPath || !/\.html?$/.test(currentPath)) return;
        ...
        if (!list) {
            try {
                const res = await fetch('/articles.html', { credentials: 'omit' });
                if (!res.ok) return;
        ...
        const idx = list.findIndex(x => x.href === currentPath);
        if (idx < 0) return;
        // articles.html is newest-first, so older = idx+1, newer = idx-1.
        const prev = idx < list.length - 1 ? list[idx + 1] : null;
        const next = idx > 0 ? list[idx - 1] : null;
        if (!prev && !next) return;

        const nav = document.createElement('nav');
        nav.className = 'pg-prev-next';
        nav.setAttribute('aria-label', 'Essay navigation');

        const mkLink = (item, label, cls) => {
            const a = document.createElement('a');
            a.href = '/' + item.href;
            a.className = 'pg-prev-next-link ' + cls;
            const lbl = document.createElement('span');
            lbl.className = 'pg-prev-next-label';
            lbl.textContent = label;
            const ttl = document.createElement('span');
            ttl.className = 'pg-prev-next-title';
        ...
        nav.appendChild(prev ? mkLink(prev, '← Previous', 'pg-prev-link') : document.createElement('span'));
        nav.appendChild(next ? mkLink(next, 'Next →', 'pg-next-link') : document.createElement('span'));
